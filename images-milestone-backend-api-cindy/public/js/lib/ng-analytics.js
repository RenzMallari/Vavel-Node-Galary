        ...
        function (ngAnalyticsService) {
            return {
                ...
                link: function ($scope) {
                    var updateChart = function(params) {
                        ...
                        var watcher = $scope.$watch(function () {
                            ...
                            if (isReady) {
                                ...
                                */
                                if(params) {
                                    if (!params.dimensions && params.startDate) {
                                        $scope.chart.query = {
                                            ... $scope.chart.query,
                                            'start-date': params.startDate,
                                            'end-date': params.endDate
                                        }
                                    } else if (params.showViews !== undefined) {
                                        if($scope.chart.chart.container != 'chart-container-1') return;

                                        let chartMetrics = 'ga:pageviews';

                                        if (params.showViews && params.showClicks) {
                                            chartMetrics = 'ga:pageviews,ga:totalEvents';
                                            $scope.chart.chart.options.colors = ['#af2f2f','#e99002'];
                                        } else if (params.showClicks) {
                                            chartMetrics = 'ga:totalEvents';
                                            $scope.chart.chart.options.colors = ['#e99002'];
                                        } else {
                                            chartMetrics = 'ga:pageviews';
                                            $scope.chart.chart.options.colors = ['#af2f2f'];
                                        }
                                        
                                        $scope.chart.query.metrics = chartMetrics;
                                    } else if (params.dimensions){
                                        if($scope.chart.chart.container != 'chart-container-1') return;
                                        $scope.chart.query.dimensions = params.dimensions;
                                    }
                                }
                    ...
                    $scope.$on('callUpdateChart', function(event, data){
                        updateChart(data);
                    });
                }
            };
        ...
        function ($templateCache, $timeout, ngAnalyticsService) {
            ...
            gapi.analytics.ready(function () {
                ...
                gapi.analytics.createComponent('ActiveUsers', {
                    ...
                    render_: function() {
                        var t = this.get();
                        this.container = 'string' == typeof t.container ? document.getElementById(t.container) : t.container, this.container.innerHTML = t.template || this.template, this.container.querySelector('b').innerHTML = this.activeUsers, this.container.querySelector('span').innerHTML = t.label;
                    },
